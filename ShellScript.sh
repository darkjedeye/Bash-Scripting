...
# Install the software that is used.
    ...
    if [[ "$answer" = "server" ]]
        then 
            read -p "Would you ike the following apps will be installed. mc sl konsole cmatrix openssh-server figlet toilet apache2 mysql-server php phpmyadmin htop sshfs neofetch fortune cowsay [yes/mo]: " SERVER
    if [[ "$SERVER" = "yes" ]]
        then
    ...
    read -p "would you like the following apps will be installed. yakuake sl kate konsole cmatrix openssh-server figlet toilet virtualbox virtualbox-dkms sshfs neofetch fortune cowsay [yes/no]: " DESKTOP
    if [[ "$DESKTOP" = "yes" ]]
    then
